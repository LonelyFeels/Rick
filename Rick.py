...
async def count(ctx):
    ...
    online_members = sum(1 for m in guild.members if m.status == discord.Status.online and if m.status == discord.Status.idle and if m.status == discord.Status.do_not_disturb)
