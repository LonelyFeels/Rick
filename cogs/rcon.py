...
class RCON(commands.Cog):
    ...
    @commands.command()
    @commands.has_any_role('Donators', 'Contributors', 'Nitro Booster')
    async def color(self, ctx, colour):
        ign = ctx.message.author.display_name
        channel = self.client.get_channel(737029420493045800)
        guild = ctx.guild
        donator = discord.utils.get(guild.roles, name='Donators')
        contributor = discord.utils.get(guild.roles, name='Contributors')
